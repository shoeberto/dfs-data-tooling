...
class CoverSpecies(Validatable):
    ...
    def validate(self):
        ...
        if self.species_known not in self.HEIGHT_OPTIONAL_SPECIES and self.average_height not in range(1, 6):
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'average height', '1-5', self.average_height))

        if self.species_known in self.DEER_INDICATOR_SPECIES:
            if None == self.count:
        ...
        if self.species_known not in self.TRILLIUM_SPECIES and None != self.number_of_stems:
        ...
        if self.species_known in self.TRILLIUM_SPECIES and None == self.number_of_stems:
