...
class GeneralTab(Tab):
    ...
    def validate(self):
        ...
        if self.deer_impact not in range(1, 6):
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'deer impact', '1-5', self.deer_impact))

        if None == self.collection_date:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'collection date', 'a date', 'None'))
...
class GeneralTabSubplot(Validatable):
    ...
    def validate(self):
        ...
        if None != self.collected and self.collected not in self.YES_NO_RESPONSE:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'collected', self.YES_NO_TEXT, self.collected))

        if None != self.fenced and self.fenced not in self.YES_NO_RESPONSE:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'fenced', self.YES_NO_TEXT, self.fenced))

        if None != self.azimuth and (self.azimuth < 0 or self.azimuth > 365):
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'azimuth', '0-365', self.azimuth))

        if None != self.distance and self.distance <= 0:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'distance', '> 0 feet', self.distance))
...
class PlotGeneralTabSubplot(GeneralTabSubplot):
    ...
    def validate(self):
        ...
        if None != self.re_monumented and self.re_monumented not in self.YES_NO_RESPONSE:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'remonumented', self.YES_NO_TEXT, self.re_monumented))

        if self.forested not in self.YES_NO_RESPONSE:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'forested', self.YES_NO_TEXT, self.forested))

        if None != self.disturbance and self.disturbance not in [0, 1]:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'disturbance', '0-1', self.disturbance))
        ...
        if None != self.disturbance_type:
            if None == self.disturbance_type or \
                ...
                validation_errors.append(FieldValidationError(self.__class__.__name__, 'disturbance type', '0-4', self.disturbance_type))

        if None != self.lime and self.lime not in self.YES_NO_RESPONSE:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'lime', self.YES_NO_TEXT, self.lime))

        if None != self.herbicide and self.herbicide not in self.YES_NO_RESPONSE:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'herbicide', self.YES_NO_TEXT, self.herbicide))
...
class FencedSubplotConditions(Validatable):
    ...
    def validate(self):
        ...
        if None != self.active_exclosure and self.active_exclosure not in self.YES_NO_RESPONSE:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'fenced subplot condition', self.YES_NO_TEXT, self.active_exclosure))

        if None != self.repairs and self.repairs not in self.YES_NO_RESPONSE:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'repairs', self.YES_NO_TEXT, self.repairs))

        if None != self.level and self.level not in range(0, 4):
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'level', '0-3', self.level))
...
class AuxillaryPostLocation(Validatable):
    ...
    def validate(self):
        ...
        if None != self.post and self.post not in [1, 2]:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'post', '1 or 2', self.post))

        if None != self.stake_type and self.stake_type not in ['Wooden', 'Rebar']:
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'stake type', 'Wooden or Rebar', self.stake_type))
...
class NonForestedAzimuths(Validatable):
    ...
    def validate(self):
        ...
        if None != self.azimuth_1 and (self.azimuth_1 < 0 or self.azimuth_1 > 359):
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'azimuth 1', '0-359', self.azimuth_1))

        if None != self.azimuth_2 and (self.azimuth_2 < 0 or self.azimuth_2 > 359):
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'azimuth 2', '0-359', self.azimuth_2))

        if None != self.azimuth_3 and (self.azimuth_3 < 0 or self.azimuth_3 > 359):
            validation_errors.append(FieldValidationError(self.__class__.__name__, 'azimuth 3', '0-359', self.azimuth_3))
